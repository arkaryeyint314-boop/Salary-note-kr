...
- The Payslip tab compares a saved Calculator snapshot with manually entered factory payslip line items. It stores one local comparison per pay month and shows Expected, Actual, and Difference values; blank payslip fields are skipped, and differences within KRW 1,000 are treated as matching.
...
- Factory Pay Rules are edited only in Profile. The Calculator's Extra pay / deduction card shows the user-entered Profile rule names with their signed amounts and the total; it has no duplicate rule-edit controls. The Factory Pay Rule editor does not expose Pay Data Source, while existing source status is preserved internally for legacy calculations.
...
- Each Shift Template also has its own Template Color. Applying a template copies that color into the calendar entry, so user-defined shifts such as `Ngt`, `3OT`, or a factory-specific Saturday shift can each have a distinct calendar color. Existing applied entries keep their copied color until the template is applied again.
...
- Calculator and Profile cards use a compact layout with reduced padding, input height, and rule/template row spacing so the main controls fit more comfortably on small screens.