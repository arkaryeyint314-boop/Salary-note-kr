...
- Factory Pay Rules are edited only in Profile. The Calculator shows the saved Profile rule names with their signed amounts and the total; it has no duplicate rule-edit controls. The Factory Pay Rule editor does not expose Pay Data Source, while existing source status is preserved internally for legacy calculations.
